...
namespace ConsistTest
{
    public static class GenericHelpers
    {
                                                                    ...
                                                                    K root_id = default(K))
        {
            foreach (var c in collection.Where(c => EqualityComparer<K>.Default.Equals(parent_id_selector(c), root_id)))
            {
                ...
                {
                    Item = c,
                    Childs = collection.GenerateTree(id_selector, parent_id_selector, id_selector(c))
                };
            }
        }


        public async static IAsyncEnumerable<TreeItem<T>> GenerateTreeAsync<T, K>(this IEnumerable<T> collection,
                                                                    Func<T, K> id_selector,
                                                                    Func<T, K> parent_id_selector,
                                                                    K root_id = default(K))
        {            
    ...
    public class TreeItem<T>
    {
        ...
        public IEnumerable<TreeItem<T>> Childs { get; set; }
    }
}
